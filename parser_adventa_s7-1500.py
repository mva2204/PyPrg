...
def get_link(ws, wb, html, icur = 1):
    ...
    for x in range(1, len(list_catalog_number) - 1):
        # print("list_price_discount[x] = ".format(list_price_discount[x]))
        if list_price_discount[x] == "Цена: по запросу":
...
f_o =open('GA Корпуса из литого алюминия ADVENTA.html', encoding="utf8")
# f_o =open('SIMATIC RACK IPC_ SIMATIC BOX IPC ADVENTA.html', encoding="utf8")
# f_o =open('SIMATIC HMI ADVENTA.html', encoding="utf8")

# f_o =open('Поиск по каталогу ADVENTA 3rw55.html', encoding="utf8")
# f_o =open('Контроллеры SIMATIC S7-400_ 400H_ Fail-Safe ADVENTA.html', encoding="utf8")
