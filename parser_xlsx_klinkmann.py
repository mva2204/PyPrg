...
class Tesli_parser(object):
    ...
    def get_list_pagination(self, html_src, mainsite, name_save_file = 'default', count_pagination_all=1):
        ...
        list_href = [html_src]
        for x in range(0, count_pagination_all):
    ...
    def get_link(self, namefile, wb, html):
        ...
        #Каталожный номер получаем
        list_catalog_number = self.get_data(soup, 'h2', "woocommerce-loop-product__title")
        print(list_catalog_number)

        #Общее описание
        list_feauture = self.get_data(soup, 'span', "category_label")
        print(list_feauture)

        #Общие данные по продукту UL Klinkman нет
        list_pruduct = self.get_data(soup, 'ul', "catalog-item__feature")
        print(list_pruduct)

        #Стоимость получаем
        catalog_item_price= self.get_data(soup, 'span', "woocommerce-Price-amount amount")
        print(catalog_item_price)
        print("catalog_item_price type = {}".format(catalog_item_price))

        soup_img = BeautifulSoup(html, 'html.parser')
        data_img = soup_img.find_all('div', {"class": "img_container"})
        list_img = []
    ...
    def save_to_excel(self, namefile, ws, list_catalog_number = [], list_feauture = [], list_pruduct= [], catalog_item_price =[], list_img =[]):
        ...
        for x in range(0,len(list_catalog_number)):
            ...
            st = str(list_catalog_number[x]).split(',')[-1]
...
name_get_html = 'https://store.klinkmann.com/ru/отдел/решения-для-управления-и-автоматизац/графические-терминалы/'
...
if flag_site_or_htmlfile:
    ...
    tesli_parser.get_list_pagination(name_get_html, 'https://store.klinkmann.com', 'Графические терминалы', 1)
else:
